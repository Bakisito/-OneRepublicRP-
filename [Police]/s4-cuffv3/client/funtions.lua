function OpenGeneralMenu()
	...
	local contextMenu = {
		...
		{ id = 2, txt = "", header = "Untie the rope on the leg", params = {event="cuff:client:action", args={ action =  "bacak_coz"}} },
		{ id = 3, txt = "", header = "Take on/off the Tape in the Mouth", params={event="cuff:client:action", args={action="bant_cikar"}} },
		{ id = 4, txt = "", header = "Release player", params={event="cuff:client:action", args={action="birak"}} },
	}

	if playerData.job.name == 'police' then 
		table.insert(contextMenu, { id = 5, txt="", header = "Get the player in the car", params={event="cuff:client:action", args={action="arac_bin"}}})
		table.insert(contextMenu, { id = 6, txt="", header = "Get out the player in the car", params={event="cuff:client:action", args={action="arac_in"}}})
		table.insert(contextMenu, { id = 7, txt="", header = "Move player", params={event="cuff:client:action", args={action="tasi"} }})
	end

	TriggerEvent("qb-menu:client:openMenu", contextMenu)
end
