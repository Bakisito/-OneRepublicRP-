	...
	--Make Cash Register Locations
		if v.payments then
			...
			exports['qb-target']:AddBoxZone("MechReceipt: "..k, v.coords.xyz, (v.w or 0.47), (v.d or 0.34), { name="MechReceipt: "..k, heading = v.coords.w, debugPoly=Config.Debug, minZ=v.coords.z-0.1, maxZ=v.coords.z+0.4 },
				{ options = { { event = "okokBilling:ToggleCreateInvoice", icon = "fas fa-credit-card", label = Loc[Config.Lan]["payments"].charge, job = v.job, img = v.img }, },
					distance = 2.0
			})
...
Config.Locations = {
	...
	{	job = "mechanic4",
		...
		nosrefill = { { coords = vec4(1171.8674, 2637.1069, 37.7750, 277.5628) } },
	...
	{	job = "mechanic5",
		...
		nosrefill = { { coords = vec4(110.91, 6630.74, 31.79, 176.76) } },
	...
	{	job = "mechanic1",
		...
		blipcolor = 81,
		discordlink = "https://discord.com/api/webhooks/1042492548251398245/WmU7gAoxYymS5jOMSlGOMT1S6eRasJb_jw6vm96iMCaUwbmuHQ9glFhphBRywR3AjK37",
		discordcolour = 2571775,
		discordimg = "https://static.wikia.nocookie.net/gtawiki/images/f/f2/GTAV-LSCustoms-Logo.png",
	},
	-- AUTO Exotic  --
	{	job = "mechanic3",
		zones = {
			vec2(528.09387207031, -165.80743408203),
			vec2(529.39984130859, -182.58442687988),
			vec2(532.16760253906, -193.23994445801),
			vec2(539.24029541016, -193.08633422852),
			vec2(539.82623291016, -212.57330322266),
			vec2(550.44580078125, -213.20350646973),
			vec2(552.07220458984, -165.20544433594)
		},
		autoClock = { enter = false, exit = false, },
		stash = { { coords = vec4(538.3455, -165.950, 54.7932, 90), w = 4.5, d = 0.9, }, },
		store = { { coords = vec4(547.8114, -182.7018, 54.7932, 180), w = 4, d = 0.8, }, }, 
		crafting = { { coords = vec4(136.71, -3051.29, 7.04, 0.0), w = 0.6, d = 1.0, }, },
		clockin = {
			{ coords = vec4(541.8090, -199.1589, 54.3, 180.0), prop = true, },  
		},
		manualRepair = { { coords = vec4(540.3610, -184.4602, 54.4, 90), prop = true, } },  --540.3610, -184.0602, 54.4824, 283.9412
		nosrefill = { { coords = vec4(548.9797, -170.2274, 54.4860, 268.4527) } }, -- 548.9797, -170.2274, 54.4860, 268.4527
		garage = { spawn = vec4(543.0393, -209.5833, 53.0382, 181.9472),
					out = vec4(545.3940, -202.9446, 54.4458, 180), -- 545.3940, -202.9446, 54.4458, 189.5752
					list = { "towtruck", "cheburek" },
					prop = true, },
		payments = { coords = vec4(541.7952, -195.2382, 54.2932, 90), --541.7952, -195.2382, 54.4932, 247.2856
			img = "<center><p><img src=https://static.wikia.nocookie.net/gtawiki/images/f/f2/GTAV-LSCustoms-Logo.png width=150px></p>",
			prop = true, },
		blip = vec3(545.0981, -179.7719, 54.4932),  
		bliplabel = "LS Tuner Shop",
		blipcolor = 81,
		discordlink = "https://discord.com/api/webhooks/1042492548251398245/WmU7gAoxYymS5jOMSlGOMT1S6eRasJb_jw6vm96iMCaUwbmuHQ9glFhphBRywR3AjK37",
		discordcolour = 2571775,
	...
	{   job = "mechanic2",
		...
		manualRepair = { { coords = vec4(-1427.3994, -450.8598, 35.9097, 302), prop = true, } },   ---1419.2427, -454.6624, 35.9097, 212.2332
		nosrefill = { { coords = vec4(-1419.2427, -454.6624, 35.9097, 195) } }, --
