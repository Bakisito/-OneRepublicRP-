--=== AMROUR ===--
RegisterNetEvent('jim-mechanic:client:applyArmour', function(data) local item = QBCore.Shared.Items["car_armor"]
	...
	if DoesEntityExist(vehicle) then
		...
		if not data then
			...
			if GetVehicleMod(vehicle, 16)+1 == GetNumVehicleMods(vehicle, 16) then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
				...
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "car_armor",}) then
		...
		elseif data == true then
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "car_armor",}) then
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if GetVehicleMod(vehicle, 12) == level then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
				SetVehicleEngineOn(vehicle, false, false, true)
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "brakes"..level+1,}) then
					...
					updateCar(vehicle)
					toggleItem(false, "brakes"..level+1, 1)	toggleItem(true, "brakes"..currentBrakes+1, 1)
					triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].installed, "success")
				else
					triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].instfail, "error")
				end
			...
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "brakes"..currentBrakes+1,}) then
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if currentSuspension == level then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
				SetVehicleEngineOn(vehicle, false, false, true)
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "suspension"..level+1,}) then
					...
					if checkSetVehicleMod(vehicle, 15, level) then
						...
						updateCar(vehicle)
						toggleItem(false, "suspension"..level+1, 1)	toggleItem(true, "suspension"..currentSuspension+1, 1)
						triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].installed, "success")
					else
						triggerNotify(nil, item.label..Loc[Config.Lan]["common"].instfail, "error")
					end
			...
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.labe, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "suspension"..currentSuspension+1,}) then
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if currentTrans == level then triggerNotify(nil, QBCore.Shared.Items["transmission"..level+1].label.." "..Loc[Config.Lan]["common"].already, "error") else
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "transmission"..level+1,}) then
					...
					if checkSetVehicleMod(vehicle, 13, level) then
						...
						updateCar(vehicle)
						toggleItem(false, "transmission"..level+1, 1) toggleItem(true, "transmission"..currentTrans+1, 1)
						triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].installed, "success")
					else
						triggerNotify(nil, item.label..Loc[Config.Lan]["common"].instfail, "error")
					end
			...
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "transmision"..currentTrans+1,}) then
...
--=== TURBO ===--
RegisterNetEvent('jim-mechanic:client:applyTurbo', function(data) local item = QBCore.Shared.Items["turbo"]
	if not enforceRestriction("perform") then return end
	if not Checks() then return end
	local vehicle = vehChecks() local above = isVehicleLift(vehicle)
    if not enforceClassRestriction(getClass(vehicle)) then return end
	if DoesEntityExist(vehicle) then
		...
		if not data then
			...
			if IsToggleModOn(vehicle, 18) then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "turbo",}) then
		...
		elseif data == true then
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "turbo",}) then
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if GetVehicleMod(vehicle, 11) == level then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "engine"..level+1,}) then
			...
			if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "engine"..currentEngine+1,}) then
...
--=== DRIFT TYRES ===--
RegisterNetEvent('jim-mechanic:client:applyDrift', function(data) local item = QBCore.Shared.Items["drifttires"]
	...
	if DoesEntityExist(vehicle) then
		...
		if GetDriftTyresEnabled(vehicle) ~= false and not data then triggerNotify(nil, Loc[Config.Lan]["common"].already, "error") else
			...
			SetVehicleModKit(vehicle, 0)
			if not data then
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag}) then
			...
			elseif data == true then
				if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag}) then SetVehicleModKit(vehicle, 0)
...
RegisterNetEvent('jim-mechanic:client:applyBulletProof', function(data) local Ped = PlayerPedId() local item = QBCore.Shared.Items["bprooftires"]
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleTyresCanBurst(vehicle) == false and not data then triggerNotify(nil, item.label..Loc[Config.Lan]["common"].already, "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			if not data then
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag}) then SetVehicleModKit(vehicle, 0)
...
--=== HARNESS ===--
RegisterNetEvent('jim-mechanic:client:applyHarness', function(data) local canEffect = true local item = QBCore.Shared.Items["harness"]
	if not enforceRestriction("perform") then return end
	if not Checks() then return end
	local vehicle = vehChecks() local above = isVehicleLift(vehicle)
	local emote = { anim = above and "idle_b" or "fixing_a_ped", dict = above and "amb@prop_human_movie_bulb@idle_a" or "mini@repair", flag = above and 1 or 16 }
	...
	lookVeh(vehicle)
	if not data then
		if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(5000,7000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "harness",}) then
			if VehicleStatus[plate]["harness"] == 1 then TriggerServerEvent("jim-mechanic:server:DupeWarn", "harness") return end
			TriggerServerEvent("jim-mechanic:server:updatePart", plate, "harness", 1)
	...
	elseif data then
		if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(5000, 7000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag, icon = "harness",}) then
			if VehicleStatus[plate].harness == 0 then emptyHands(Ped) TriggerServerEvent("jim-mechanic:server:DupeWarn", "harness") return end
			TriggerServerEvent("jim-mechanic:server:updatePart", plate, "harness", 0)
			qblog("`"..item.label.." - harness` removed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
			updateCar(vehicle)
...
--=== ANTILAG ===---
RegisterNetEvent('jim-mechanic:client:applyAntiLag', function(data) local item = QBCore.Shared.Items["antilag"]
	if not enforceRestriction("perform") then return end
	if not Checks() then return end
	local vehicle = vehChecks() local above = isVehicleLift(vehicle)
    if not enforceClassRestriction(getClass(vehicle)) then return end
		...
		if VehicleStatus[plate].antiLag == 1 and not data then triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].already, "error") else
			...
			if not data then
				if progressBar({label = Loc[Config.Lan]["common"].installing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag}) then SetVehicleModKit(vehicle, 0)
					...
					TriggerServerEvent("jim-mechanic:server:updatePart", plate, "antiLag", 1)
					qblog("`antilag - "..item.label.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
					updateCar(vehicle)
					toggleItem(false, "antilag")
					triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].installed, "success")
				else
					triggerNotify(nil, item.label..Loc[Config.Lan]["common"].remfail, "error")
				end
			elseif data == true then
				if progressBar({label = Loc[Config.Lan]["common"].removing..item.label, time = math.random(7000,10000), cancel = true, anim = emote.anim, dict = emote.dict, flag = emote.flag}) then SetVehicleModKit(vehicle, 0)
					...
					TriggerServerEvent("jim-mechanic:server:updatePart", plate, "antiLag", 0)
					qblog("`antilag - "..item.label.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
					updateCar(vehicle)
					toggleItem(true, "antilag")
					triggerNotify(nil, item.label.." "..Loc[Config.Lan]["common"].removed, "success")
				else
					triggerNotify(nil, item.label..Loc[Config.Lan]["common"].remfail, "error")
				end
