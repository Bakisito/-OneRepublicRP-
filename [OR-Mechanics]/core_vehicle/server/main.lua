...
AddEventHandler( "core_vehicle:canInstall", function(partType, part)
        ...
        if Config.UsingCoreInventory then
             ...
             local xItems = exports['core_inventory']:getItems('content-' .. xPlayer.PlayerData.citizenid, part)
            if xItems[1] ~= nil then
            if xItems[1].amount > 0 then
...
AddEventHandler(
    ...
    function(part, health)
        ...
        xPlayer.Functions.AddItem(part, 1, nil, {health = health})
        end
    end
)
