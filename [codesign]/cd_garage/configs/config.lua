...
Config.Language = 'ES' --[ 'EN' / 'CZ' / 'DE' / 'DK' / 'ES' / 'FI' / 'FR' / 'NL' / 'PT' / 'SE' / 'SK' ]   You can add your own locales to locales.lua, but be sure to update the Config.Language to match it.
...
Config.Impound = {
    ...
    Impound_Fee = { --This is the price players pay for their vehicle to be unimpounded.
        method = 'vehicles_data', --[ 'default' / 'vehicles_data' ] Read below for more info on each of these 2 options. (Config.VehiclesData.ENABLE must be enabled if you want to use 'vehicles_data').
        default_price = 1000, --If 'default' method is chosen, then it will be a set price to unimpounded any vehicle. (eg., $1000 fee).
        vehiclesdata_price_multiplier = 15 --If 'vehicles_data' method is chosen, the unimpounded vehicle price will be a % of the vehcles value. (eg., 1% of a $50,000 car would be a $500 fee).
    }
}
...
Config.PropertyGarages = {
    ...
    only_showcars_inpropertygarage = true --Do you want the inside garage to only show the vehicles which are currently stored in a property garage. (this works for inside garage only, even with this enabled all the cars will show in the outside UI).
}
...
Config.JobVehicles = {
    ...
    Locations = {
        ...
        ['police'] = { --If you choose to add more tables here for more jobs, they must be the jobs name, not the label.
            --MISSION ROW PD
            [1] = {coords = vector3(436.93, -961.49, 23.94), spawn_coords = vector4(431.75, -956.89, 23.34, 183.42), distance = 5, garage_type = 'car', method = 'personalowned'}, --Mission Row PD (cars)
            [2] = {coords = vector3(455.26, -985.27, 43.69), spawn_coords = vector4(448.84, -981.55, 44.08, 274.68), distance = 5, garage_type = 'air', method = 'regular'}, --Mission Row PD (helipad)
            --SANDY PD
        ...
        ['ambulance'] = {
            --PILLBOX HOSPITAL
            [1] = {coords = vector3(320.41, -569.89, 28.8), spawn_coords = vector4(332.61, -576.68, 28.8, 339.68), distance = 10, garage_type = 'car', method = 'personalowned'}, --Pillbox Hospital (cars)
    ...
    RegularMethod = {
        ...
        ['ambulance'] = {
            ...
            [2] = {job = 'ambulance', spawn_max = true, plate = 'EMS', job_grade = 0, garage_type = 'air', model = `emsair`},
        },
    }
}
