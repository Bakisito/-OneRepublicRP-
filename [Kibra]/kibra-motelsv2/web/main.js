        ...
        if (event.data.type === "OpenMotel"){
            ...
            if (MotelRooms === false){
               ...
               <center><button class="ButtonGroupElement" id="AllMotelRooms" type="button">Todas las habitaciones:</button>
                <button id="MyMotelRooms" class="ButtonGroupElement" type="button">Mis habitaciones</button></center>
                `
            }
            ...
            for (let data = 0; data < event.data.Motel.Rooms.length; data++) {
                ...
                if (element.Owner === "") {
                    ...
                    <div class="Room">
                        <h1>Room ${odaid}</h1>
                        <button id="RentMotelRoom" data-mid="${event.data.id}" data-roomid="${data+1}" class="RentButton">($${event.data.Motel.RoomRentPrice}) Alquilar</button>
                    </div>
                ` 
                }
            };
            ...
            for (let data = 0; data < event.data.Motel.Rooms.length; data++) {
                ...
                if (element.Owner === "") {
                    ...
                    <div class="Room">
                        <h1>Room ${odaid}</h1>
                        <button id="RentMotelRoom" data-mid="${event.data.mid}" data-roomid="${data+1}" class="RentButton">($${event.data.Motel.RoomRentPrice}) Rent Room</button>
                    </div>
                ` 
                }
            };
            ...
            for (let data = 0; data < event.data.data.length; data++) {
                ...
                if(ayarlama === false){
                    ...
                    <div class="RoomBig">
                        <h1>Room ${element.roomid}</h1>
                        <button motel="${element.roomid}" id="LeaveMotelRoom" class="RentButton2">Leave the Room</button>
                        <button motel="${element.roomid}" id="CopyMotelKey" class="RentButton2">Copy Room Key</button>
                        <button motel="${element.roomid}" id="NewMotelKey" class="RentButton2">Change Room Key</button>
                    </div>`
                }
            };
            $('.MotelRoomsTable').html("");
