...
local Webhook = 'https://discord.com/api/webhooks/1092237792047792212/54M4p4umxA_RqdMESM5lMFlRqRhcMzWg_nAOGp24FExwEYE0kY4Lyt9tRNB15lmy7ol2' -- PUT YOUR WEBHOOK LINK HERE
...
if Config.EnableMeCommand then
	RegisterCommand(Config.MeCommand, function(source, args, rawCommand)
		...
		local icon = 'icons'
		message= string.sub(message,1,Config.MaxLength)
		...
		if chatsHidden[source] == nil then
			...
			TriggerClientEvent('okokChat:me2', -1, message, source, icon)
			TriggerEvent('rcore_cam:me', source, message)
...
if Config.EnableDoCommand then
	RegisterCommand(Config.DoCommand, function(source, args, rawCommand)
		...
		if chatsHidden[source] == nil then
			...
			TriggerClientEvent('okokChat:do2', -1, message, source, icon)
			TriggerEvent('rcore_cam:do', source, message)
...
if Config.EnableDoFijoCommand then
	RegisterCommand(Config.DoFijoCommand, function(source, args, rawCommand)
		...
		local playerNameAdmin = playerName.." ["..source.."]"
		local length = string.len(Config.OOCCommand)
		local message = rawCommand:sub(length + 4)

		if mutedList[source] ~= nil then
			TriggerClientEvent('chat:addMessage', source, {
				template = '<div class="chat-message muted"><i class="'..Config.TimeOutIcon..'"></i> <b><span style="color: #faa479">'..Config.MessageTitle..'</span>&nbsp;<span class="time">{1}</span></b><div class="message">You are muted for <b>{0}</b></div></div>',
				args = { SecondsToClock(mutedList[source].timeLeft), time }
			})
			return
		end
		...
		if chatsHidden[source] == nil then
			showToClosePlayers(xPlayer, function(players)
				TriggerClientEvent('chat:addMessage', players, {
					template = '<div class="chat-message dofijo"><i class="'..Config.DoFijoIcon..'"></i> <b><span style="color: #faa479">['..Config.DoFijoMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></b><div class="message">{1}</div></div>',
					args = { playerName, message, time }
				})
			end)
			--TriggerClientEvent('okokChat:3dme', -1, message, source, "dofijo")
			...
			TriggerClientEvent('okokChat:triggerDisplay', -1, source, args, "dofijo")
			...
			if Webhook ~= '' then
				...
				local data = {
					...
					identifier = identifierlist.license:gsub("license2:", ""),
					discord = "<@"..identifierlist.discord:gsub("discord:", "")..">",
					type = Config.WebhookText['dofijo'],
					message = message,
				}
				discordWebhook(data)
			end
...
RegisterCommand('doff', function(source, args, rawCommand)
	if source == 0 then return end
	args = table.concat(args, '')
    TriggerClientEvent('okokChat:triggerDoOff',-1,source, args, "dofijo")
	TriggerClientEvent('chat:addMessage', source, { args = { "^5DOFi ", "^1El modo 'do' ha sido desactivado." } })
end, false)
