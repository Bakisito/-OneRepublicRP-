...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		...
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "medikit", 1) then
			if oldMan then
			...
			TriggerEvent('qb-log:server:CreateLog', 'ambulancejob', 'Jugador baneado', 'red', string.format('%s fue baneado por %s for %s', GetPlayerName(src), 'qb-ambulancejob', "Intentar revivirse a sí mismos o a otros jugadores"), true)
			DropPlayer(src, 'Has sido baneado permanentemente por el servidor por: Exploiting')
		end
	end
end)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'El médico ya ha sido avisado', 'error')
    end
end)
