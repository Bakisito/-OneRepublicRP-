...
QBCore.Commands.Add('hora', Lang:t("command.hora.help"), {}, false, function(source)
    ...
    local fecha = os.date('%d/%m/%Y', timestamp)
    local hora = os.date('%I:%M:%S %p', timestamp)
    RconPrint(string.format('[%s] La fecha y hora actuales son %s %s', playerName, fecha, hora))
    TriggerClientEvent('QBCore:Notify', source, Lang:t('info.hora_info', {value = fecha .. ' ' .. hora}))
end, 'user')
