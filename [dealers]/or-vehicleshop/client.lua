...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    -- TriggerServerEvent('or-vehicleshop:server:addPlayer', citizenid)
    -- TriggerServerEvent('or-vehicleshop:server:checkFinance')
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
   -- TriggerServerEvent('or-vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'or-vehicleshop:client:showVehOptions'
        }
    }
}
...
--         params = {
--             event = 'or-vehicleshop:client:getVehicles'
--         }
--     }
-- }
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'or-vehicleshop:client:TestDriveReturn'
        }
    }
}
        ...
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if secondsLeft >= tonumber(1000 * testDriveTime) - 20 or GetPedInVehicleSeat(NetToVeh(testDriveVeh), -1) ~= PlayerPedId() then
                    TriggerServerEvent('or-vehicleshop:server:deleteVehicle', testDriveVeh)
                    testDriveVeh = 0
                    inTestDrive = false
                ...
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'or-vehicleshop:client:TestDrive',
                            }
                        },
                        {
                            ...
                            params = {
                                isServer = true,
                                event = 'or-vehicleshop:server:buyShowroomVehicle',
                                args = {
                        ...
                        --     params = {
                        --         event = 'or-vehicleshop:client:openFinance',
                        --         args = {
                        --             price = getVehPrice(),
                        --             buyVehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                        --         }
                        ...
                        -- },
                        {
                            ...
                            params = {
                                event = 'or-vehicleshop:client:vehCategories',
                            }
                        },
                    }
                    Wait(1000)
                ...
                while insideShop and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'or-vehicleshop:client:openIdMenu',
                                args = {
                        ...
                        },
                        {
                            ...
                            icon = "fa-solid fa-coins",
                            params = {
                                event = 'or-vehicleshop:client:openCustomFinance',
                                args = {
                                    price = getVehPrice(),
                                    vehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                                }
...
-- Events
RegisterNetEvent('or-vehicleshop:client:homeMenu', function()
    exports['qb-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('or-vehicleshop:client:showVehOptions', function()
    exports['qb-menu']:openMenu(vehicleMenu, true, true)
end)

RegisterNetEvent('or-vehicleshop:client:TestDrive', function()
...
RegisterNetEvent('or-vehicleshop:client:customTestDrive', function(data)
...
RegisterNetEvent('or-vehicleshop:client:TestDriveReturn', function()
...
RegisterNetEvent('or-vehicleshop:client:vehCategories', function()
	local catmenu = {}
    local categoryMenu = {
        {
            ...
            params = {
                event = 'or-vehicleshop:client:homeMenu'
            }
        }
    }
    ...
    for k, v in pairs(catmenu) do
        categoryMenu[#categoryMenu + 1] = {
            ...
            params = {
                event = 'or-vehicleshop:client:openVehCats',
                args = {
                    catName = k
                }
            }
...
RegisterNetEvent('or-vehicleshop:client:openVehCats', function(data)
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = "fa-solid fa-angle-left",
                ...
                for _, shop in pairs(QBCore.Shared.Vehicles[k]["shop"]) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                isServer = true,
                                event = 'or-vehicleshop:server:swapVehicle',
                                args = {
                                    toVehicle = v.model,
...
-- RegisterNetEvent('or-vehicleshop:client:openFinance', function(data)
...
--         TriggerServerEvent('or-vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
--     end
-- end)

-- RegisterNetEvent('or-vehicleshop:client:openCustomFinance', function(data)
...
--         TriggerServerEvent('or-vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.playerid)
--     end
-- end)

RegisterNetEvent('or-vehicleshop:client:swapVehicle', function(data)
...
RegisterNetEvent('or-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        local veh = NetToVeh(netId)
        exports['LegacyFuel']:SetFuel(veh, 100)
...
-- RegisterNetEvent('or-vehicleshop:client:getVehicles', function()
--     QBCore.Functions.TriggerCallback('or-vehicleshop:server:getVehicles', function(vehicles)
--         local ownedVehicles = {}
...
--                     icon = "fa-solid fa-car-side",
--                     params = {
--                         event = 'or-vehicleshop:client:getVehicleFinance',
--                         args = {
--                             vehiclePlate = plate,
--                             balance = v.balance,
...
-- RegisterNetEvent('or-vehicleshop:client:getVehicleFinance', function(data)
--     local vehFinance = {
--         {
--             header = Lang:t('menus.goback_header'),
--             params = {
...
--             icon = "fa-solid fa-hand-holding-dollar",
--             params = {
--                 event = 'or-vehicleshop:client:financePayment',
--                 args = {
--                     vehData = data,
...
--             params = {
--                 isServer = true,
--                 event = 'or-vehicleshop:server:financePaymentFull',
--                 args = {
...
-- RegisterNetEvent('or-vehicleshop:client:financePayment', function(data)
--     local dialog = exports['qb-input']:ShowInput({
...
--         TriggerServerEvent('or-vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
--     end
-- end)

RegisterNetEvent('or-vehicleshop:client:openIdMenu', function(data)
    local dialog = exports['qb-input']:ShowInput({
    ...
    if dialog then
        ...
        if data.type == 'testDrive' then
            TriggerServerEvent('or-vehicleshop:server:customTestDrive', data.vehicle, dialog.playerid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('or-vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.playerid)
        end
    end
end)
