...
-- RegisterNetEvent('or-vehicleshop:server:addPlayer', function(citizenid)
--     financetimer[citizenid] = os.time()
-- end)

-- Deduct stored game time from player on logout
-- RegisterNetEvent('or-vehicleshop:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('or-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('or-vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('or-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('or-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('or-vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('or-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('or-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
-- RegisterNetEvent('or-vehicleshop:server:financePayment', function(paymentAmount, vehData)
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
...
-- RegisterNetEvent('or-vehicleshop:server:financePaymentFull', function(data)
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
--     local cash = player.PlayerData.money['cash']
--     local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('or-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('or-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
-- RegisterNetEvent('or-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
--     local src = source
...
-- RegisterNetEvent('or-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
...
--                 0
--             })
--             TriggerClientEvent('or-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
...
-- RegisterNetEvent('or-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
--     local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
-- RegisterNetEvent('or-vehicleshop:server:checkFinance', function()
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
