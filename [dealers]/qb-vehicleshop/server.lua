...
AddEventHandler('qb-vehicleshop:client:orderVehicle', function(vehicle, data)
    ...
    local data = {
        ['content'] = 'Se ha encargado un nuevo vehículo.',
        ['embeds'] = {
            {
                ['title'] = 'Se ha realizado un nuevo pedido de vehículos',
                ['description'] = '¡Alguien ha pedido un vehículo nuevo!',
    ...
    PerformHttpRequest(webhookUrl, function(statusCode, response, headers)
        TriggerClientEvent('QBCore:Notify', src, 'Su pedido ha sido realizado', "success")
...
RegisterServerEvent('qb-vehicleshop:server:buy-stock', function(vehicle)
    ...
    if account >= discount  then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Vehículo comprado con éxito', "success")
    else
        TriggerClientEvent('QBCore:Notify', src, 'El concesionario no tiene suficiente dinero', "error")
    end
end)
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if vehicle.stock < 1 then
        TriggerClientEvent('QBCore:Notify', src, 'Agotado', 'error')
        return
    end
