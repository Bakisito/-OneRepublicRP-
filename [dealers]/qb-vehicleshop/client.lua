...
RegisterNetEvent('CardealerToggle:Duty', function()
    onDuty = not onDuty
...
AddEventHandler('qb-vehicleshop:client:orderVehicle', function(message)
    ...
    local discord_message = "Se ha pedido un vehículo en " .. insideShop .. "." -- customize the message as needed
...
CreateThread(function()
    for k, v in pairs(Config.Shops) do
        ...
        if v.showDuty then
            ...
            exports['qb-target']:AddBoxZone("duty" .. k, vector3(v.Duty), 1.5, 1, {
                name = "duty" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.Duty.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "CardealerToggle:Duty",
                        icon = "fa fa-clipboard",
                        label = "Entrar/Salir de servicio",
                        canInteract = function()
                            return (PlayerData.job.name == Config.Shops[k]['Job'])
                        end
