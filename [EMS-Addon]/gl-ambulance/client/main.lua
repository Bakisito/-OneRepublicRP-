...
RegisterNetEvent('gl-ambulance:interactBag', function()
    if PlayerJob.name == 'ambulance' or PlayerJob.name == 'police' then
        exports['qb-menu']:openMenu({{
            header = 'Bolsa médica',
            txt = 'Artículos almacenados',
            icon = "medbag",
            isMenuHeader = true
        }, {
            header = 'Pinzas',
            txt = 'Se utiliza para extraer balas',
            icon = "tweezers",
            params = {
            ...
            header = 'Kit de sutura',
            txt = 'Se utiliza para suturar heridas',
            icon = "suturekit",
            params = {
                event = "gl-ambulance:getItem",
                args = {
            ...
            header = 'Crema para quemaduras',
            txt = 'Tus madres lo van a necesitar',
            icon = "burncream",
            params = {
                event = "gl-ambulance:getItem",
                args = {
            ...
            header = 'Desfibrilador',
            txt = 'Para restablecer los latidos del corazón',
            icon = "defib",
            params = {
                event = "gl-ambulance:getItem",
                args = {
...
RegisterNetEvent('gl-ambulance:useItem', function(itemName)
    ...
    if itemName == 'medikit' then
        ...
        QBCore.Functions.Notify('Ha utilizado 1x MediKit', 'success', 2000)
    end
end)
...
RegisterNetEvent('gl-ambulance:sendPulseBack', function(target)
    ...
    local minPulse = 60
    local maxPulse = 80
    local status = 'Parecen estar bien.'
    QBCore.Functions.TriggerCallback('gl-ambulance:isPlayerDead', function(isdead)
        if isdead then
            minPulse = 0
            maxPulse = 0
            status = 'No hay latido'
        else
            if isShot then
                minPulse = 120
                maxPulse = 200
                status = 'Hemorragia por aparentes heridas de bala'
            end
            if isMeleed then
                minPulse = 90
                maxPulse = 115
                status = 'Presenta laceraciones o hematomas profundos'
            end
            if isBurned then
                minPulse = 20
                maxPulse = 50
                status = 'Quemaduras en todo el cuerpo'
            end
        end
...
RegisterNetEvent('gl-ambulance:checkInNancy', function()
    if exports['qb-ambulancejob']:GetDoctorCount() >= Config.MinimalDoctor then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        local ped = PlayerPedId()
        local bed = Config.BedLocs[math.random(#Config.BedLocs)]
        QBCore.Functions.Notify('Ha sido trasladado a una cama para tratamiento.', 'success')
        Panitaencama = true
        ...
        -- QBCore.Functions.Notify('Ya estás bien, deja de meterte en líos.', 'success')
    end
end)
...
RegisterNetEvent('gl-ambulance:useGrandmas', function()
    QBCore.Functions.Notify('Coño mi niñ@, te me bregaron...', 'primary', 5000)
    Wait(5000)
            ...
            QBCore.Functions.TriggerCallback('gl-ambulance:isPlayerDead', function(isdead)
                if isdead then
                    ...
                    if doMath <= 20 then
                        ...
                        QBCore.Functions.Notify('No funcionó el desfibrilador y lo rompiste.', 'error')
                    end
...
RegisterNetEvent('gl-ambulance:useTheBed', function()
    ...
    if exports['qb-ambulancejob']:GetDoctorCount() >= Config.MinimalDoctor then
        QBCore.Functions.Notify('Hay médicos de servicio, ve a recepción y pregunta por ellos.')
    else
        QBCore.Functions.Notify('Le han trasladado a una cama para tratamiento.')
        Panitaencama = true
...
CreateThread(function()
    ...
    exports['qb-target']:AddGlobalVehicle({
        options = {{
            ...
            label = 'Sacar al jugador del vehículo',
            job = 'ambulance',
            ...
            icon = "fas fa-ambulance",
            label = 'Poner al jugador dentro del vehículo',
            job = 'ambulance',
            canInteract = function(entity)
                local player, distance = QBCore.Functions.GetClosestPlayer()
                if player ~= -1 and distance <= 3.0 then
    ...
    exports['qb-target']:AddGlobalPlayer({
        options = {{
            ...
            icon = "fas fa-heartbeat",
            label = "Poner en bolsa para cadáveres",
            canInteract = function(entity)
                local p = promise.new()
                QBCore.Functions.TriggerCallback('gl-ambulance:isPlayerDead', function(isdead)
                    p:resolve(isdead)
...
exports('Panitaencama', function () return Panitaencama end)
