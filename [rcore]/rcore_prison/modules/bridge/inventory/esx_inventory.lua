CreateThread(function()
    IsResourceLoaded('es_extended', function(state, inventory)
        if state then
            if IsScriptLoaded('inventory') then
                return Utils.Debug('inform', 'Cheeza inventory detected, disabling default ESX inventory.')
            end
                    ...
                    if prefix and label then
                        ...
                        if money and money > 0 then
                            if label == 'pocketMoney' then
                                xPlayer.addMoney(money, 'return')
                            elseif label == 'black_money' then
                                xPlayer.addAccountMoney(label, money, "return")
                            end
                ...
                for indexName, data in pairs(accounts) do
                    ...
                    if prefix then
                        if label and label == 'pocketMoney' then
                            xPlayer.removeMoney(amount, "return")
                            SetResourceKvpInt(prefix, amount)
                        elseif label and label == 'black_money' then
                            xPlayer.removeAccountMoney(label, amount, "return")
                            SetResourceKvpInt(prefix, amount)
                        end
                    end
                end
