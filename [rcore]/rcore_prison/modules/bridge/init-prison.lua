...
if IsDuplicityVersion() then
    ...
    InternalCharacterLoaded = function(source, loadType)
        ...
        if CanPrisonerBeReleased(charId) then
            ...
            if prisoner.state == 'jailed' then
                ...
                if CWUsers[source] and CWUsers[source].position then
                    TriggerClientEvent('rcore_prison:setBlipWaypoint', source, source, {
                        coords = CWUsers[source].position,
    ...
    if Prison.Commands.State then
        ...
        RegisterCommand(Prison.Commands.Jail, function(source, args, rawCommand)
            ...
            if time and time > 0 and target then
                if GetPlayerPed(target) > 0 and isPrisonerAtGuard(serverId, target) then
                    TriggerEvent('rcore_prison:startSentence', {
                        issuedBy = {
                            serverId = serverId
                        },
    ...
    Bridge.Notify = function(serverId, data)
        if Prison.DefaultNotify then
            ...
            TriggerClientEvent(Prison.FrameworkTriggers.notify, serverId, data.description)
        end
    end
else
    Bridge = {}
    ...
    Bridge.GetSkinType = function()
        ...
        if not retval then
            ...
            retval = 'unk'
            LevelDebug(3, 'Unknown clothing script, loading Skip outfit selection!')
        end
    ...
    if Prison.Notify.DefaultNotify then
        ...
        Bridge.Notify = function(data)
            return dbg.debug('[Notify] You need to hook own notify')
        end
    end
    ...
    GetRealResourceByMetadata = function(name)
        local retval = nil
    ...
    Bridge.SelectOutfit = function(plyPed, data)
        IsResourceLoaded(Bridge.GetSkinType(), function(state, func)
            if state then
                local retval = GetRealResourceByMetadata('qb-clothing')

                if (retval and retval == 'fivem-appearance') or Bridge.GetSkinType() == 'fivem-appearance' then
                    exports['fivem-appearance']:setPedAppearance(plyPed, data)
                elseif (retval and retval == 'illenium-appearance') or Bridge.GetSkinType() == 'illenium-appearance' then
                    Utils.tprint(data)
                    exports['illenium-appearance']:setPlayerAppearance(data)
                elseif Bridge.GetSkinType() == 'skinchanger' then
                    local curData = data.components and data.components.skin

                    if curData then
                        TriggerEvent('skinchanger:loadSkin', curData)
                    end
                elseif (retval and retval == 'qb-clothing') or Bridge.GetSkinType() == Prison.FrameworkTriggers.resourceNameClothing then
                    Bridge.SetOutfit('selectOutfit', data)
                end
            else
                dbg.error('Failed to jail clothing for Prisoner, skinDriver unk!!!')
            end
        end)
    end
    ...
    Bridge.HandleResetPrisoner = function(data, state)
        ...
        local plyPed = PlayerPedId()
        local fetchSkinState = data and Utils.tablesize(data) <= 0 and 'failed to fetch user skin' or 'skin loaded'
        local retval = GetRealResourceByMetadata('qb-clothing')
        ...
        if state == 'jailed' then
            IsResourceLoaded(Bridge.GetSkinType(), function(state, func)
                if state then
                    dbg.debug('Player skin data state [%s]', fetchSkinState)
                    ...
                    elseif (retval and retval == 'qb-clothing') or Bridge.GetSkinType() == Prison.FrameworkTriggers.resourceNameClothing then
                        Bridge.SetOutfit('release', data)
                    end
                else
                    dbg.error('Failed to restored player clothing, unk skinDriver!!')
                end
            end)
        end
