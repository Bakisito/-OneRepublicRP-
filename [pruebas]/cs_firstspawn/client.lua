		...
		if IsThisModelACar(taxiModel) then
			...
			if not DoesEntityExist(taxiVeh) then
                ...
                if IsPedInVehicle(PlayerPedId(), taxiVeh, 1) then
                    DoScreenFadeIn(250)
                    exports['okokNotify']:Alert('One Republic RP', 'Bienvenido a One Republic RP', 5, 'info', playSound)
    ...
    while true do
        if Active then
            ...
            if dist <= 10 then
                ...
                exports['okokNotify']:Alert('One Republic RP', 'Hemos llegado a nuestro destino', 5, 'sucess', playSound)
